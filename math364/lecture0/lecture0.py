...
class Content(Scene):
    def construct(self):
        ...
        trigonometry = Text("علم المثلثات", font=ARABIC_FONT, font_size=40, color=LOGO_GREEN)
        point1 = Dot([6, 1, 0])
        trigonometry.next_to(point1, LEFT)
        self.play(FadeIn(point1))
        self.play(Write(trigonometry, reverse=True))
        self.add(trigonometry)
        trigonometry_image = SVGMobject("../../assets/images/trigonometry.svg", stroke_color=LOGO_GREEN, stroke_width=5).scale(0.5)
        trigonometry_image.next_to(trigonometry, LEFT)
        self.play(Write(trigonometry_image))

        functions = Text("تحليل الدوال", font=ARABIC_FONT, font_size=40, color=LOGO_RED)
        point2 = Dot([6, -0.5 , 0])
        functions.next_to(point2, LEFT)
        self.play(FadeIn(point2))
        self.play(Write(functions, reverse=True))
        self.add(functions)
        functions_image = SVGMobject("../../assets/images/functions.svg", stroke_color=LOGO_RED, stroke_width=5).scale(0.5)
        functions_image.next_to(functions, LEFT)
        self.play(Write(functions_image))

        limits_and_derivatives = Text("الاشتقاق والنهايات", font=ARABIC_FONT, font_size=40, color=LOGO_YELLOW)
        point3 = Dot([6, -2 , 0])
        ...
        limits_and_derivatives_image = SVGMobject("../../assets/images/limits_and_derivatives.svg", stroke_color=LOGO_YELLOW, stroke_width=5).scale(0.5)
        limits_and_derivatives_image.next_to(limits_and_derivatives, LEFT)
        self.play(Write(limits_and_derivatives_image))
